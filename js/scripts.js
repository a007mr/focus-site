...
var plansClass = 'feature__btn__meeting-4';
var icebreskersClass = 'feature__btn__culture-1';
var moodClass = 'feature__btn__culture-2';
var kudosClass = 'feature__btn__culture-3';
var okrSlackNotifyClass = 'feature__btn__okr-4';
var okrSlackRemindClass = 'feature__btn__okr-5';
var okrSlackUpdateClass = 'feature__btn__okr-6';
...
// Убрал кнопку All-in-On/Check-ins с помощью замены на true
// условия на 101 и 134 строках
var product = true;
...
function switchImage(e) {
	...
	var cultureActiveBtn = document.querySelector('.feature__btn__active-culture');
 	...
 	} else if (clickedBtn.classList.contains(plansClass)) {
 		imageMeeting.src='https://usefocus.co/img/guides/slack-settings-3.png';
 		activeBtn = meetingActiveBtn;
 		type = 'meeting';
 	...
 	} else if (clickedBtn.classList.contains(okrSlackNotifyClass)) {
 		imageOkr.src='https://usefocus.co/img/guides/slack-okr-1.png';
 		activeBtn = okrActiveBtn;
 		type = 'okr';

 	} else if (clickedBtn.classList.contains(okrSlackRemindClass)) {
 		imageOkr.src='https://usefocus.co/img/guides/slack-okr-2.png';
 		activeBtn = okrActiveBtn;
 		type = 'okr';

 	} else if (clickedBtn.classList.contains(okrSlackUpdateClass)) {
 		imageOkr.src='https://usefocus.co/img/guides/slack-okr-3.png';
 		activeBtn = okrActiveBtn;
 		type = 'okr';
...
// console.log('=== utm_campaign 3', utm_campaign);
...
function getCookie(cname) {
    ...
    // console.log('=== we are in getCookie for cname', cname);
    // console.log('=== decodedCookie in getCookie', decodedCookie);
    // console.log('=== ca in getCookie', ca);
    ...
    for(var i = 0; i <ca.length; i++) {
        ...
        if (c.indexOf(name) == 0) {
    		// console.log('=== name.length in IF', name.length);
    		// console.log('=== c.length in IF', c.length);
...
function setCookie(cname, cvalue) {
    ...
    document.cookie = `${cname}=${cvalue};${expires};path=/;SameSite=None; Secure;domain=usefocus.co`;
    // Fallback for legacy browsers: https://web.dev/samesite-cookie-recipes/
    // document.cookie = `${cname}=${cvalue};${expires};path=/;Secure`;
    document.cookie = `${cname}=${cvalue};${expires};path=/;Secure;domain=usefocus.co`;

    // document.cookie = `${cname}=${cvalue};${expires};path=/;Secure;domain=localhost:3000`;

    // console.log('=== we are in setCookie for cname', cname);
    // console.log('=== cvalue', cvalue);
    // console.log('=== expires in setCookie', expires);
    // console.log(`result 1: ${cname}=${cvalue};${expires};path=/;SameSite=Strict`);
    // console.log(`result 2: ${cname}=${cvalue};${expires};path=/;Secure`);
}
